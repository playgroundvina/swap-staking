...
export default function TokenSwapScreen({ Web3, Contracts, Accounts }) {
  ...
  const swapTokens = async () => {
    if (finalAmount > 0) {
      ...
      if (tokenSelected === 'ABC') {
        const receipt = await contracts[1].methods
          .approve(TokenSwapAddress, switchAmount)
          .send({ from: accounts[0] });
        const result = await contracts[0].methods
          .swapTKA(switchAmount)
          .send({ from: accounts[0] });
      } else {
        const receipt = await contracts[2].methods
          .approve(TokenSwapAddress, switchAmount)
          .send({ from: accounts[0] });
        const result = await contracts[0].methods
          .swapTKX(switchAmount)
          .send({ from: accounts[0] });
      }
    } else {
  ...
  const buyTokensABC = async () => {
    try {
      const receipt = await contracts[1].methods
        .buyTokens(numOfTokenA)
        .send({ from: accounts[0], value: numOfTokenA * abcTokenPrice });
    } catch (err) {
  ...
  const buyTokensXYZ = async () => {
    try {
      const receipt = await contracts[2].methods
        .buyTokens(numOfTokenX)
        .send({ from: accounts[0], value: numOfTokenX * xyzTokenPrice });
    } catch (err) {
      alert('Transaction Failed due to the following Error' + err);
    }
  };
    ...
    <div class="container mt-5">
      <nav class="navbar navbar-expand-lg navbar-light bg-dark rounded-3 ">
        <div class="container-fluid">
          ...
          <label class="badge bg-primary fs-4 rounded">
            TokenABC Balance: {tokenABalance} Tokens
          </label>
          <label class="badge bg-primary fs-4 rounded">
            TokenXYZ Balance: {tokenXBalance} Tokens
          </label>
        </div>
      </nav>
      <div class="d-flex mt-3 input-group">
        ...
        ></input>
        <button
          class="btn-danger text-center form-control"
          type="submit"
          onClick={() => {
            buyTokensABC(numOfTokenX);
          }}
        >
          buy Token ABC
        </button>
      ...
      <div class="input-group mt-3">
        ...
        <label class="alert alert-info mt-3">
          1 ABC = {ratio} XYZ, Fees: {fees}% Expected to get:{finalAmount}
        </label>
      </div>
    </div>
  );
